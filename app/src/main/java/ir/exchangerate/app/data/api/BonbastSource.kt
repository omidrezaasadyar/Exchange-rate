...
/**
 * bonbast.com is unreachable from most Iranian ISPs without a VPN. With a VPN
 * the homepage renders a table where each cell carries an id like #usd1
 * (sell) and #usd2 (buy). We render the page in WebView, wait for the JS to
 * populate prices, then read the cells.
 */
class BonbastSource(
    ...
    private val sellSelectors = mapOf(
        Currency.USD to listOf("#usd1", "td#usd1", "span#usd1"),
        Currency.EUR to listOf("#eur1", "td#eur1", "span#eur1"),
        Currency.OMR to listOf("#omr1", "td#omr1", "span#omr1"),
    )
    ...
    override suspend fun fetch(currencies: List<Currency>): Map<Currency, Result<Rate>> {
        ...
        val html = runCatching {
            scraper.fetchRenderedHtml(
                ...
                readyJsExpression =
                    "document.querySelector('#usd1') && document.querySelector('#usd1').textContent.replace(/[^0-9]/g,'').length > 3",
                minDelayMs = 1500L,
        ...
        return currencies.associateWith { currency ->
            runCatching {
                val sellToman = findFirstNumber(doc, sellSelectors[currency].orEmpty())
